...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({ 
        ...
        repository_url: this.git.repository_url,
        repository_name: this.git.repository_name,
      }),
  ...
  methods: {
    submit() {
      this.$inertia.put(route("git.update", [this.project[0].id, this.enviroment[0].id, this.$props.git.id]), this.form);
    },
  }
}
