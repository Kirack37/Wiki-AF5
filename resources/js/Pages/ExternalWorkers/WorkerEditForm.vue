  ...
  <app-layout>
    <template #header>
        <h2 class="font-semibold text-xl text-white leading-tight">
            Edición del trabajador externo
        </h2>
    </template>
        <div class="container mx-auto my-10 bg-white">
          <form @submit.prevent="submit">
            ...
            <div class="mt-14">
              <label for="notes">Notas sobre el trabajador externo:</label>
        ...
        <button class="bg-yellow-500 rounded p-4 mt-8" type="submit"> Editar el trabajador externo </button>
      </form>
    </div>
  </app-layout>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      form: this.$inertia.form({
        ...
        strengths: this.worker.strengths,
        weaknesses: this.worker.weaknesses,
  ...
  methods: {
    submit() {
      this.$inertia.put(
        route("externalworkers.update", this.$props.worker.id),
        this.form
      );
    },
  }
