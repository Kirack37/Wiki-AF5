  ...
  <app-layout>
    ...
    <div class="container mx-auto my-10">
      ...
      <div class="recent-items">
        ...
        <div v-for="trello in trellos" :key="trello.id" class="item-card flex border-2 rounded border-double border-gray-300 mb-3 p-3 bg-white">
          <div class="item-data flex-1">
            <h4 class="text-black">{{ trello.url }}</h4>
          </div>
          <div class="item-data flex-1">
            <p>{{ moment(trello.created_at).format("DD-MM-YYYY") }}</p>
          </div>
          <div class="item-data mr-12">
            <inertia-link
              ...
              method="get"
              as="button"
              :href="route('trello.edit', [project[0].id, enviroment[0].id, trello.id])">
              <font-awesome-icon :icon="['fas', 'edit']" size="lg" />
            </inertia-link>
          </div>
          <div class="item-data mr-12">
            <button class="text-red-600" @click="doDelete(trello.id)">
...
export default {
  ...
  methods: {
    ...
    resetQuery() {
      this.$inertia.replace(this.route('trello.index', [this.project[0].id, this.enviroment[0].id, '']))
    ...
    searchFor() {
      ...
      this.$inertia.replace(this.route('trello.index', {project: this.project[0].id, enviroment: this.enviroment[0].id, term: this.term}))
      
    },
    async doDelete(trello) {
      const ok = await this.$refs.confirmDialogue.show({
          title: 'Eliminar Trello',
          message: 'Confirma que deseas borrar este Trello. Esta acción no puede deshacerse.',
          okButton: 'Eliminar',
      })
      if (ok) {
          this.$inertia.delete(this.route('trello.destroy', [this.project[0].id, this.enviroment[0].id, trello]));
      } 
  },
  },
};
